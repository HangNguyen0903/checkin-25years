  ...
  return (
    <aside className={`h-screen bg-white transition-all duration-300
      ...
      <div className="flex items-center justify-between p-4 relative">
        ...
        >
          {collapsed ? <ChevronRight size={16} className="hover:bg-gray-200 rounded-full" /> : <ChevronLeft size={16} />}
        </button>
      </div>
      <nav className="flex-1 px-4 py-2 space-y-2">
        {routes.map((r) => (
          ...
          >
            <div className ='flex gap-2 text-sm items-center'> {r.icon} <span> {!collapsed && r.label}</span></div>
          </Link>
        ))}
      </nav>
    </aside>
